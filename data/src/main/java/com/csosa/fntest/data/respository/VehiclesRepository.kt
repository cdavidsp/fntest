...
class VehiclesRepository(
    ...
    override suspend fun getVehiclesbyBounds(apiKey: String, location: Coordinate, bounds: Bounds): Flow<List<Vehicle>> = flow {
        ...
        if (mustBeCallToBE ||  queries.isEmpty()) {
            ...
            val origins = String.format("%f,%f", location.latitude, location.longitude)
            ...
            for(call in 0 until numberOfCalls) {
                ...
                for (vehicleResponse in initialList) {
                    ...
                    destinations = destinations + "|" + String.format("%f,%f", vehicleResponse.coordinate.latitude, vehicleResponse.coordinate.longitude)
                }

                delay(50)
                val distanceMatrix = googleMapsApiService.getDistanceMatrix(origins, destinations, apiKey)
